...
// Brighton Laiman: University of California, San Diego
int
TclCommand_addTaperedFiberSection(ClientData clientData, Tcl_Interp * interp, int argc, 
								  TCL_Char ** argv, TclModelBuilder * theBuilder);
			       ...
			       TCL_Char **argv, TclModelBuilder *theTclBuilder)
{
	...
	*/
	...
	// Brighton Laiman: University of California, San Diego
		...
		return TclCommand_addTaperedFiberSection(clientData, interp, argc, argv, theTclBuilder);
...
// Brighton Laiman: University of California, San Diego
int
buildTaperedSection(Tcl_Interp * interp, TclModelBuilder * theTclModelBuilder, int secTag,
...
// Brighton Laiman: University of California, San Diego
int
TclCommand_addTaperedFiberSection(ClientData clientData, Tcl_Interp * interp, int argc, TCL_Char ** argv,
								  TclModelBuilder * theTclModelBuilder)
{
	...
	// build the fiber section (for analysis)
	if (buildTaperedSection(interp, theTclModelBuilder, secTag, isTorsion, G, hRatio) != TCL_OK) 
	{
...
int
TclCommand_addTaperedFiber(ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char **argv, TclModelBuilder *theTclModelBuilder)
{
	// check if a section is being processed
	if (currentSectionTag == 0) 
	{
		opserr << "WARNING subcommand 'TaperedFiber' is only valid inside a 'section' command\n";
		return TCL_ERROR;
	}
	...
	if (argc < 6) 
	{
		opserr << "WARNING invalid num args: fiber yLoc zLoc area matTag tP\n";
		return TCL_ERROR;
	}

	SectionRepres *sectionRepres = theTclModelBuilder->getSectionRepres(currentSectionTag);
	...
	if (Tcl_GetDouble(interp, argv[1], &yLoc) != TCL_OK) 
	{
		opserr << "WARNING invalid yLoc: fiber yLoc zLoc area matTag tP\n";
		return TCL_ERROR;
	}

	if (Tcl_GetDouble(interp, argv[2], &zLoc) != TCL_OK) 
	{
		opserr << "WARNING invalid zLoc: fiber yLoc zLoc area matTag tP\n";
		return TCL_ERROR;
	}

	if (Tcl_GetDouble(interp, argv[3], &area) != TCL_OK) 
	{
		opserr << "WARNING invalid area: fiber yLoc zLoc area matTag tP\n";
		return TCL_ERROR;
	}

	if (Tcl_GetInt(interp, argv[4], &matTag) != TCL_OK) 
	{
		opserr << "WARNING invalid matTag: fiber yLoc zLoc area matTag tP\n";
		return TCL_ERROR;
	}
	...
	if (NDM == 2) 
	{
		// Tapered Fiber Section does not support 2D models... throw error message here
		opserr << "WARNING Tapered Fiber Section is not supported for 2D models!\n";
		return TCL_ERROR;
	...
	else if (NDM == 3) 
	{
		...
		theFiber = new UniaxialFiber3d(numFibers, *material, area, fiberPosition, dValue, tP);
		if (theFiber == 0) 
		{
			opserr << "WARNING unable to allocate fiber \n";
			return TCL_ERROR;
...
//               opserr<<"HeightValue[i]"<<HeightValue[i];
        ...
        else if (NDM == 3) 
		{
            ...
            for (i = numSectionRepresFibers; i < numFibers; i++) 
			{
                ...
                fiber[i] = new UniaxialFiber3d(k, *material, fibersArea(k), fiberPosition, dValue, fiberstP(k));

                if (fibersArea(k) < 0)
                    opserr << "ERROR: " << fiberPosition(0) << " " << fiberPosition(1) << endln;
